...
pygame.display.set_caption("DinoJao")
...
class Dinosaur:
    ...
    def update(self, userInput):
        ...
        if 40 <= int(ldr_value) <= 69 and not self.dino_jump :  #Jump
            self.dino_duck = False
            self.dino_run = False
            self.dino_jump = True

        elif 94 <= int(ldr_value) <= 100 and not self.dino_jump :  #Duck
            self.dino_duck = True
            self.dino_run = False
            self.dino_jump = False
            
        elif 70 <= int(ldr_value) <= 93 and not self.dino_jump :  #Run
            self.dino_duck = False
            self.dino_run = True
            self.dino_jump = False
...
#CREATE OBSTACLE (BIRD or CACTUS)
class Obstacle:
...
def main_menu(): #MAIN MENU
    global switch18_value, switch38_value
    switch18_value=0
    switch38_value=0
    x=0
    while True:
...
#                 obstacles.append(Bird(BIRD))
        ...
        if len(obstacles) <= 2: #INSERT OBJECTS
            if int(switch18_value) == 1 and state:#INSERT CACTUS RIGHT BUTTON
                if random.randint(0, 1) == 0:
                    obstacles.append(SmallCactus(SMALL_CACTUS))
                elif random.randint(0, 1) == 1:
                    obstacles.append(LargeCactus(LARGE_CACTUS))
                state = False
            elif int(switch18_value) == 0:
                state = True
            
            if int(switch38_value) == 1 and state1:#INSERT BIRD LEFT BUTTON
                obstacles.append(Bird(BIRD))
                state1 = False
            elif int(switch38_value) == 0:
                state1 = True
...
def menu(death_count):#MENU WHETHER TO MAIN OR RESTART
    global points
    global FONT_COLOR
    global switch18_value, switch38_value, state, sw_value, ldr_value
    switch18_value=0
    switch38_value=0
    sw_value = 0
    x=0
    ...
    while run:
        ...
        if death_count == 0:
            text = font.render("PRESS [Right Button] TO START", True, (0,0,0))
            if int(switch18_value) == 1 and state1:
                main()
                state1 = False
            elif int(switch18_value) == 0:
                state1 = True
        elif death_count >= 0:
            text = font.render("PRESS [Left Button] TO RESTART", False, (0,0,0))
            ...
            MENU_BUTTON2 = Button(image=pygame.image.load("assets/Options Rect.png"), pos=(640, 550), text_input="BACK TO MENU", font=get_font(45), base_color="White", hovering_color="Green")
            MENU_TEXT = get_font(100).render("GAME OVER", True, "#b68f40")
            MENU_RECT = MENU_TEXT.get_rect(center=(640, 100))
            SCREEN.blit(MENU_TEXT, MENU_RECT)
            
            SELECT_MOUSE_POS = pygame.mouse.set_pos(640, 550) #BACKTOMENU
            for button in [MENU_BUTTON2]:
                button.changeColor(MENU_MOUSE_POS2)
                button.update(SCREEN)
        
            for event in pygame.event.get():
                SELECT_MOUSE_POS = pygame.mouse.set_pos(640, 550)
                if event.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
                yee3 = 1024
                state3 = True
                if switch18_value == '1' and state3 :
                    yee3 +=1
                    state3 = False
                elif switch38_value == '0':
                    state3 = True
                if yee3 == pygame.MOUSEBUTTONDOWN:
        ...
        for event in pygame.event.get():
            ...
            if int(switch38_value) == 1 and state2:
                main()
                state2 = False
            elif int(switch38_value) == 0:
                state2 = True
